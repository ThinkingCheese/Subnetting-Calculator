...
class IP_Calculator():
  ...
  def GatherVerifiyInput(self):
    ...
    try:
      ...
      if len(self.splitIpaddress) != 4:
        print(
          "The amount of octets is too low or input is invalid. Please try again.\n"
        )
        return False
      ...
      if self.subnetMask[0] == "/":
        ...
        if slashNum < 0 or slashNum > 32:
          print(
            f"Slash notation number {self.subnetMask} is invalid. Please try again.\n"
          )
          return False
        return True
      ...
      if len(split_mask) == 4:
        slash = 0
        bitRange = ("0", "128", "192", "224", "240", "248", "252", "254",
                    "255")
        for i, octet in enumerate(split_mask):
          num = int(octet)
          if num < 0 or num > 255 or prevNum < num:
            print("Octet(s) of the subnet mask is invalid. Try again.\n")
            return False
        ...
        return True
      print(
        "Subnet mask error; cannot identify subnet mask format, Please try again."
      )
      return False

    except ValueError as e:
      print(
        f"Input given failed validation because of incorrect values. Please try again.\n\nError: {e}\n"
      )
      return False
    except Exception as e:
      print(
        f"Input validation failed; please try again.\n\nError: {e}\n")  #later
      return False
    finally:
  ...
  def CalculateIPaddress(self):
    ...
    if slashNotation >= 31:
      print(
        "The address given is a host address; subnet mask too high to proceed."
      )
      return
    currentIPClass = None
    ...
    networkAddress[classLocation] = str(
      int(
        math.floor(int(self.splitIpaddress[classLocation]) / hostSubnet) *
        hostSubnet))
    ...
    lastHost[classLocation] = str(
      int(lastHost[classLocation]) + int(hostSubnet) - 1)
    ...
    nextSubnet[classLocation] = str(
      int(nextSubnet[classLocation]) + hostSubnet)
    lastHost[3] = "254"
